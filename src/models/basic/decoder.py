...
class LinearDecoder(nn.Module):
    ...
    def __init__(self, use_manhattan=True, mahattan_weight=1.0,pseudoimage_channels: int = 64):
        super().__init__()
        self.use_manhattan = use_manhattan
        self.manhattan_weight = mahattan_weight

        self.offset_encoder = nn.Linear(3, 128)

        # 解码器输入维度：pseudoimage_channels*4 (拼接特征) + 128 (偏移编码)
        decoder_input_dim = pseudoimage_channels * 2 + 128

        self.decoder = nn.Sequential(
            nn.Linear(decoder_input_dim, 32), nn.GELU(),
            nn.Linear(32, 3))

        # 曼哈顿约束层
        if self.use_manhattan:
            self.manhattan_classifier = nn.Sequential(
    ...
    def apply_manhattan_constraint(self, flow, axis_weights):
        """应用曼哈顿距离约束"""
        # 创建硬约束：仅保留主导轴向的流
        dominant_axis = torch.argmax(axis_weights, dim=-1, keepdim=True)
        mask = torch.zeros_like(flow)
        mask.scatter_(-1, dominant_axis, 1.0)

        # 混合约束：硬约束 + 原始流
        constrained_flow = flow * mask * self.manhattan_weight + \
                           flow * (1 - self.manhattan_weight)
    ...
    def forward_single(self, before_pseudoimage: torch.Tensor,
        ...
        # 组合所有特征 [N, 128 + 128]
        combined_features = torch.cat([concatenated_vectors, point_offsets_feature], dim=1)

        # 原始流预测 [N, 3]
        raw_flow = self.decoder(combined_features)

        # 应用曼哈顿约束
        if self.use_manhattan:
            axis_weights = self.manhattan_classifier(combined_features)
            flow = self.apply_manhattan_constraint(raw_flow, axis_weights)
        else:
            flow = raw_flow
