...
from . import ConvWithNorms,SelfAttenetion,CrossAttention
...
class FastFlow3DUNet(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        num_heads = 8
        self.bottleneck_self_attn = SelfAttenetion(bottleneck_channels,num_heads)
        self.bottleneck_cross_attn = CrossAttention(bottleneck_channels,num_heads)
    ...
    def forward(self, pc0_B: torch.Tensor,
        ...
        # --- 2. 在瓶颈处应用注意力 (核心改动) ---
        # 可选的自注意力: 首先在每个分支内部优化特征
        pc0_R_refined = self.bottleneck_self_attn(pc0_R)
        pc1_R_refined = self.bottleneck_self_attn(pc1_R)

        # 双向交叉注意力: pc0关注pc1，同时pc1也关注pc0
        pc0_from_pc1 = self.bottleneck_cross_attn(pc0_R_refined, pc1_R_refined)
        pc1_from_pc0 = self.bottleneck_cross_attn(pc1_R_refined, pc0_R_refined)

        Rstar = torch.cat([pc0_from_pc1, pc1_from_pc0],
